...
void CArk::UpdateVirtualSensor(CKilobotEntity &c_kilobot_entity){
    ...
    if(!m_bOptionsLookUpTableSent){
        ...
        */
        ...
        /* Discovery Messages*/
        //if the KB encountered option and the update cd has pass
        if( ( tEncounteredOption[GetKilobotId(c_kilobot_entity)] != -1) 
        && (m_fTimeInSeconds - tLastTimeMessagedDiscovery[GetKilobotId(c_kilobot_entity)]> MinTimeBetweenTwoMsg ))
        {
            if(m_fTimeInSeconds>=m_tOptions[tEncounteredOption[GetKilobotId(c_kilobot_entity)]].AppearanceTime 
            && (m_fTimeInSeconds<m_tOptions[tEncounteredOption[GetKilobotId(c_kilobot_entity)]].DisappearanceTime 
            || m_tOptions[tEncounteredOption[GetKilobotId(c_kilobot_entity)]].DisappearanceTime<=0))
            {
